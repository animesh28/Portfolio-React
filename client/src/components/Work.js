...
function Work() {
  let jmdS21Ref,
    jmdMacRef = useRef(null);
  let natoursGifVid,
    natoursImg = useRef(null);
  let rudraMacRef,
    rudraTabRef = useRef(null);
  let maharajaMacRef,
    maharajaMobRef = useRef(null);
  let crazyMacRef,
    crazyTabRef = useRef(null);
  let disneyMacRef,
    disneyMobRef = useRef(null);
  let teslaMacRef,
  ...
  useEffect(() => {
    ...
    dot2.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot2 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
    ...
    dot3.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot3 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
    ...
    dot4.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot4 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
    ...
    dot5.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot5 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
    ...
    dot6.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot6 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
    ...
    dot7.addEventListener("click", (e) => {
      const pos = Math.ceil(
        document.body.scrollHeight * (t1.labels.fillDot7 / t1.duration())
      );
      gsap.to(window, { duration: 3, scrollTo: pos, ease: "linear" });
    });
  ...
  const [textData, setTextData] = useState({
    name: "Natours",
    descrip: "Exciting tours and travels",
    url: "https://animesh28.github.io/Natours/",
  });
  const setNatours = () => {
    setTextData({
      name: "Natours",
  ...
  const setDisney = () => {
    setTextData({
      name: "Disney+ Clone",
      descrip: "OTT Platform",
      url: "https://disneyplus-clone-fe126.web.app/",
    });
  ...
  const setJmd = () => {
    ...
    setTextData({
      name: "JMD Developers",
      descrip: "Construction | Real Estate | Joint Venture",
      url: "https://jmddevelopers.com/",
    });
  };
  const setRudra = () => {
    ...
    setTextData({
      name: "Rudra Motors",
      descrip: "Tata Motors Dealership",
      url: "https://rudramotor.com/",
    });
  };
  const setMaharaja = () => {
    ...
    setTextData({
      ...
      descrip: "Photography | Cinematography",
      url: "https://maharaja-studios.netlify.app/",
    });
  };
  const setCrazy = () => {
  ...
  const setTesla = () => {
    ...
    setTextData({
      name: "Tesla Clone",
      descrip: "Electric Cars | Solar Panels",
      url: "https://animesh-tesla-clone.netlify.app/",
    });
  };
      ...
      <ImageWrapContainer>
        ...
        <ImageWrap2 id="img-wrap_2">
          <DisneyMac
            src={disneyMac}
            alt="Disney-Mac"
            ref={(el) => (disneyMacRef = el)}
          />
          <DisneyMob
            src={disneyMob}
            alt="Disney-Mob"
            ref={(el) => (disneyMobRef = el)}
          />
        </ImageWrap2>
        ...
        <ImageWrap7 id="img-wrap_7">
          <TeslaMac
            src={teslaMac}
            alt="Tesla-Mac"
            ref={(el) => (teslaMacRef = el)}
          />
          <TeslaTab
            src={teslaTab}
            alt="Tesla-tab"
            ref={(el) => (teslaTabRef = el)}
          />
        </ImageWrap7>
      </ImageWrapContainer>
      <SvgSlider />
...
const TextWrap = styled.div`
  ...
  background: #00824d;
  z-index: 19;
  box-shadow: inset 2rem -2rem 4rem rgba(0, 0, 0, 0.3);
  position: relative;
